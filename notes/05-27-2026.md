...
requirements, iteration and improvement upon job parameters, and re-running the
job with the updated parameters.

### IDEAS Project

The IDEAS project is a collaborative effort that is currently being lead by Dr.
Victor Mateevitsi with collaborators located in the Viz. lab, ANL MCS + CPS
...
However, more broadly my work is to integrate some degree of agentic AI software
...
Kickoff meeting with Silvio, Jakub, and Mengjiao. The project is framed as
integrating agentic AI into the larger IDEAS visualization loop: scientist
submits an inquiry, an agent submits a job to Aurora, a second agent reviews the
output, and the loop iterates until the result matches the scientist's
specifications. Current IDEAS outputs render in Unity/Unreal for VR/AR viewing,
with raw data compressed to INRs (owned by UC Davis) and multi-variable
gaussians. My summer scope is to extend the output path to VTK/ParaView (via
ParaView MCP) and to insert agentic tooling at the scene parameterization and
hyper-parameter tuning seams. Homework: draft week 1–10 goals and identify at
least three submission venues.

### How This Builds on Prior Work

This is the project kickoff — there are no prior daily notes in scope. The note
establishes the stakeholders (Silvio, Jakub, Mengjiao), the parent project
(IDEAS, led by Victor Mateevitsi), the candidate stack (VTK, ParaView, ParaView
MCP, LangChain/LangGraph), and the explicit out-of-scope items (INRs are UC
Davis's responsibility). Everything that follows in this folder is downstream of
the framing decisions captured here.

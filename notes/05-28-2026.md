...
## Homework From [05-27-2026](05-27-2026.md)
...
## 3:00 PM Meeting W/ Jakub, Silvio, + Menjiang

### Summary

Nicholas has completed nearly all of his Workday tasks. Upon checking, he still
has to enroll in his benefits.

Jakub is having trouble with his laptops WiFi card when connecting to the ANL
WiFi network. With respect to Jakub's project, he may be using the developer
version of Kokkos as it often has bug fixes not immediatly merged into the main
version. He will also need to take care not to use the JIT as that can add
performance overhead. Finally, he will need to decide on whether to implement
his Kokkos backend as a standalone application or as an additional wrapper to
`espresso`. As a requirement of his university, he may need Silvio's physical
signature to sign off on that he completed his work accordingly.

Both Nicholas and Jakub's task lists can be improved by implementing task level
parallelism; if there are easy tasks that can be completed weeks in advance,
then they should be completed. Finally, both Nicholas and Jakub's Ph.D. advisors
need to sign off on the work that they are planning to do this summer.
Nicholas's Ph.D. advisor (George) has given his approval via Slack on the
initial task plan.

### Nicholas's Task Presentation

Nicholas showed his task presentation generated in assistance with Anthropic
Claude via Argo. While it recieved wide support for the tasks, concerns were
raised about the:

- Amount of tasks,
- Ordering of tasks regarding the task level parallelism, and
- Confidence regarding the tooling and the amount of deliverables per week.

### Assigned Homework for 05-29-2026
...
Produced the first full 10-week plan (M1.1 through M10.6) with explicit scope
exclusions (INRs, MFAs, KANs, homomorphic data representations, neural
rendering) and identified five candidate submission venues centered on
SC-affiliated workshops (SC Poster, WORKS, AI-on-HPC, AI4S, AgenticAI4HPC). A
10am meeting with George confirmed Argo for agents, ALCF Inference for batch,
and named MCP authoring/integration as the central SE skill for the summer. A
3pm meeting with Silvio, Jakub, and Mengjiao reviewed the task presentation and
flagged concerns about task volume, ordering, and lack of task-level
parallelism. Homework: get on every compute resource (Polaris, Crux, Sophia,
Argo API, ALCF Batch Inference), wire Argo + ALCF Inference into OpenCode,
install ParaView + ParaView MCP, and read the proposal, ChatVis, and Mathai
papers.

### How This Builds on [05-27-2026](05-27-2026.md)

Converts the open-ended 05-27 homework ("draft weeks 1–10" and "find 3 venues")
into a concrete plan-of-record and a long checklist of access and tooling tasks.
The weekly breakdown operationalizes the IDEAS-integration framing from 05-27
into an MVP-then-multi-agent-then-benchmark arc, and the George/Silvio meetings
add two specific constraints absent on 05-27: Argo vs. ALCF Inference role
separation, and an explicit critique that the task list needs parallelism and
tighter scoping.

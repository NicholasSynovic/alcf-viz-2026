...
[ ] Access compute resources [ ] Polaris [ ] Crux [ ] Sophia [ ] Argo API [ ]
ALCF Batch Inference [ ] From each compute resource, get access to ArgoAPI [ ]
From each compute resource, get access to ALCF Batch Inference [ ] Connect
ArgoAPI to OpenCode [ ] Connect ALCF Batch Inference to OpenCode [ ] Join
OpenCode_AI Slack channel [ ] Join AI_help_no_bad_questions Slack channel [ ]
Reorder task list to support task level parallelism [ ] Read provided papers [ ]
Project proposal [ ] ChatVis [ ] Mavish Paper [ ] Install Paraview [ ] Connect
to Paraview MCP via OpenCode [ ] Start work on Week 1 tasks

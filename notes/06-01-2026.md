...
## Homework from [05-29-2026](05-29-2026.md)
...
- **Tasks:**
    ...
    - [ ] M1.3 Draft a one-page project charter specifying:
        ...
        - [x] Which LLM backends to evaluate (proprietary and open-source - see
              M2.5).
...
- **Tasks:**
    ...
    - [x] M2.8 Get access to ALCF Viz Lab GitHub and look for other ALCF related
          organizations
...
Read the project proposal and ChatVis (Mathai still pending), reordered the task
list for parallelism, and re-embedded the full weekly breakdown in this note.
The 11am meeting with Silvio and Victor was the first major scoping
intervention: the existing multi-agent/orchestration plan is too ambitious for
ten weeks and needs to narrow to MCP tooling plus A2A as an extension of
ChatVis. Three alternative directions were proposed — (1) benchmarking a
ParaView Docs MCP tool vs. ChatVis JSON-RAG, (2) building a scientific
provenance/logging teaching tool for IDEAS, (3) a user study (blocked behind ≥1
year of IRB review) — and the team leaned toward option (1) as the most
tractable summer deliverable. Wrote pros/cons for the first two options; the
closing meeting added: focus on a ParaView MCP PoC ASAP and start generating
ParaView vizes via Argo against the benchmark examples. New homework includes
OhMyAgents for OpenCode, `argo-shim`, a DGX Spark user (via Victor), and TGV CFD
data (via Saumil).

### How This Builds on [05-29-2026](05-29-2026.md)

First scope-down since the 05-28 plan was drafted. Where 05-29 was almost
entirely about unblocking access and getting ParaView MCP to run, today the
project's research question itself is being narrowed: the multi-agent
coordination and large-scale benchmarking weeks from 05-28 are now flagged as
scope creep, and the proposed deliverable shrinks to a focused MCP-vs-RAG
comparison built on top of ChatVis. The ParaView MCP runtime failure from 05-29
is still open and is unchanged; the difference is that it now sits on the
